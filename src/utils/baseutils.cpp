...
QCursor setCursorShape(QString cursorName, int colorIndex) {
    ...
    if (cursorName == "start") {
        ...
        if (ration <= 1)
        {
            startPix = QIcon(":/img/mouse_style/shape/start_mouse.svg").pixmap(START_SIZE);
            customShape = QCursor(startPix, 8, 8);
        } else
        {
            startPix = QIcon(":/img/mouse_style/shape/start_mouse@2x.svg").pixmap(START_SIZE);
            customShape = QCursor(startPix, int(8*ration), int(8*ration));
        }

    } else if (cursorName == "rotate") {
        QPixmap rotateCursor  = QIcon(":/img/mouse_style/shape/rotate_mouse.svg").pixmap(ARROW_SIZE);
        ...
        QPixmap lineCursor  = QIcon(":/img/mouse_style/shape/line_mouse.svg").pixmap(ARROW_SIZE);
