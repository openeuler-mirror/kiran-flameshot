...
QString ConfigHandler::PFormatValue() {
    QString pformat = "jpg";
    if (m_settings.contains(QStringLiteral("pformat"))) {
        pformat = m_settings.value(QStringLiteral("pformat")).toString();
    }
    return pformat;
}

void ConfigHandler::setPFormatValue(const QString &picture_format) {
        m_settings.setValue(QStringLiteral("pformat"), picture_format);
}

QString ConfigHandler::defaultSaveTip() {
        QString saveTip = "desktip";
        if (m_settings.contains(QStringLiteral("saveTip"))) {
            saveTip = m_settings.value(QStringLiteral("saveTip")).toString();
        }
        return saveTip;
}

void ConfigHandler::setSaveTip(const QString &savetip) {
        m_settings.setValue(QStringLiteral("saveTip"), savetip);
}
